  ...
  <form method="post" name="formusers" id="formusers" class="topBefore" onsubmit="return validate_user();" action="index.php?page=controller_users&op=update">
    ...
    <?php
        if(isset($error)){
         ...
         <input  type="text" name="id" id="id"  placeholder="id" readonly="readonly" value="<?php echo $user['id'];?>" />
        <br>
     ...
     <input name="correo" type="text" id="correo" class="correo" placeholder="email"  value="<?php echo $user['email'];?>" />
 
      <br>
      <span id="e_fecha" class="styerror"></span>
       <input id="demo1" type="text" name="fecha" class="fecha" placeholder="fecha" readonly="readonly" value="<?php echo $user['fecha'];?>" /> 
